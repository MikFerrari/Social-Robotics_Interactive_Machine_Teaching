classdef Human_agent
    ...
    methods
        ...
        function obj = Human_agent(resp_type,optim,det,true_thresh,mu,sigma)
            %HUMAN_AGENT Construct an instance of this class
            obj.response_type = resp_type;
        ...
        %%% FUNCTIONS TO PROVIDE AN ANSWER %%%
        ...
        function label_prob = teacher_response(obj,input)
            switch obj.response_type
                case "Infallible"
                    label_prob = response_exact(obj,input);
                case "Fallible"
                    label_prob = response_imperfect(obj,input);
                case "Manual"
                    label_prob = response_manual(obj);
            end
        end


        %%% FUNCTIONS TO PROVIDE INPUTS %%%
        ...
        function [input_sample,obj] = provide_input_seed(obj,lo_bound,hi_bound,delta)
            ...
            if obj.count_positive == 0 && random_choice < 0.33
                input_sample = min([hi_bound obj.true_threshold+delta]);
                obj.count_positive = 1;
            elseif obj.count_negative == 0 && random_choice > 0.66
                input_sample = max([lo_bound obj.true_threshold-delta]);
